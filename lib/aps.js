...
const APS_OSS  = "https://oss.api.autodesk.com"; // <- noul host pentru OSS v2
...
export async function ensureBucket(token, bucketKey = process.env.APS_BUCKET, policyKey = "persistent") {
  ...
  const det = await fetch(`${APS_OSS}/oss/v2/buckets/${bucketKey}/details`, {
    method: "GET",
    headers: {
  ...
  if (det.status === 404) {
    const r = await fetch(`${APS_OSS}/oss/v2/buckets`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
      ...
      body: JSON.stringify({
        ...
        region: "EMEA", // important pentru Europa
      }),
    });
