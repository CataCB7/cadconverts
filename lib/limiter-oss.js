// lib/limiter-oss.js
// Rate-limit pe IP stocat în APS OSS/S3 (folosind proxy-ul tău pentru signed URLs)
...
function todayYMD() {
  // reset zilnic după UTC (simplu & suficient pentru MVP)
...
// Scrie JSON în OSS folosind /upload (cu Content-Length corect)
...
export async function checkAndConsumeIp(req, limit = 2) {
  if (!PROXY_BASE_URL || !APS_BUCKET) {
    // dacă nu e configurat, nu blocăm (MVP dev-safety)
